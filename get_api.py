...
def get_instrument_info(typ, symbol = '', url = ''):
	...
	r = requests.get(url=url1, headers=headers, timeout=3)
...
if __name__ == "__main__":
	#get_fundamentals_info的使用演示
	stocks = get_fundamentals_info(['MSFT','FB'])
	print(stocks[0].headquarters_city+"\n"+stocks[1].sector)


	#get_instrument_info的使用演示
	stock = get_instrument_info(typ = 'symbol',symbol = 'MSFT')
	print(stock.symbol)
	stock = get_instrument_info(typ = 'url',url = 'https://api.robinhood.com/instruments/50810c35-d215-4866-9758-0ada4ac79ffa/')
	print(stock.name)


	#get_quote_info的使用演示
	stocks = get_quote_info(['MSFT','FB'])
	print(stocks[0].ask_price+"\n"+stocks[1].symbol)


	#get_market_info的使用演示
	market = get_market_info(typ = 'hours',mic = 'XNYS', date = "2018-08-09")
	print(market.opens_at)	
	market = get_market_info(typ = 'mic',mic = 'XNYS')
	print(market.name)
